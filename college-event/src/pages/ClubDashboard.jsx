...
function ClubDashboard() {
  ...
  const { fetchingMyClubs, fetchingRequests, updatingMembership, fetching } = useSelector(selectClubsLoading);
  const { fetchingMy: fetchingMyEvents } = useSelector(selectEventsLoading);
  
  const [activeTab, setActiveTab] = useState('overview');
  const roleBadgeColors = badgeColors[user?.role] || badgeColors.ORGANIZER;

  // Separate events into upcoming and recent
  const { upcomingEvents, recentEvents } = useMemo(() => {
    const now = new Date();
    const upcoming = myEvents.filter((e) => new Date(e.dateTime) >= now);
    const recent = myEvents.filter((e) => new Date(e.dateTime) < now);
    return {
      upcomingEvents: upcoming.sort((a, b) => new Date(a.dateTime) - new Date(b.dateTime)),
      recentEvents: recent.sort((a, b) => new Date(b.dateTime) - new Date(a.dateTime)),
    };
  }, [myEvents]);
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case 'overview':
        ...
        const upcomingMyEvents = upcomingEvents.length;
    ...
    <div style={pageContainerStyle}>
      <div style={contentWrapperStyle}>
        <div style={{ marginBottom: '32px' }}>
          <h1 style={{ fontSize: '32px', fontWeight: '700', marginBottom: '8px', color: '#1a202c' }}>Club Organizer Dashboard</h1>
